...
const DikshitDetailsForm = ({ formData, updateFormData, nextStep, prevStep, handleRegister }) => {
  ...
  // Initialize form with formData on component mount and when formData changes
  useEffect(() => {
    if (formData) {
      // Set isDikshit state
      const isDikshitValue = formData.isDikshit !== undefined ? formData.isDikshit : null;
      setIsDikshit(isDikshitValue);

      // Set dikshitDetails state - handle both dikshitDetails and dikshitDetail formats
      let detailsArray = [];
      if (formData.dikshitDetails && Array.isArray(formData.dikshitDetails)) {
        detailsArray = formData.dikshitDetails;
      } else if (formData.dikshitDetail && Array.isArray(formData.dikshitDetail)) {
        // Convert from API format to component format
        detailsArray = formData.dikshitDetail.map((detail, index) => ({
          id: index + 1,
          dikshitName: detail.dikshitName || "",
          worldlyName: detail.worldlyName || "",
          guruName: detail.guruName || "",
      ...
      if (detailsArray.length > 0) {
        setDikshitDetails(detailsArray);
      }

      // Set form values
      form.setFieldsValue({
        isDikshit: isDikshitValue,
        dikshitDetails: detailsArray.length > 0 ? detailsArray : dikshitDetails,
      });
    }
  }, [formData]); // Remove form and dikshitDetails from dependencies to avoid infinite loops

  // Save form data whenever isDikshit or dikshitDetails change
  useEffect(() => {
    const currentValues = {
      isDikshit,
      dikshitDetails,
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Use current state for submission instead of form values
      ...
      // Format the data in the requested structure
      const formattedData = formatDataForSubmission(submissionData);
