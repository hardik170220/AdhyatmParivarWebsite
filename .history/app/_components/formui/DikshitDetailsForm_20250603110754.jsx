...
const DikshitDetailsForm = ({ formData, updateFormData, nextStep, prevStep, handleRegister }) => {
  ...
  // Function to format data as array
  const formatDataAsArray = (formValues) => {
    const { isDikshit, dikshitDetails } = formValues;
    
    // Base array with isDikshit status
    const dataArray = [
      {
        field: "isDikshit",
        label: "परिवार में दीक्षा हुई है?",
        value: isDikshit,
        displayValue: isDikshit ? "हाँ" : "ना"
      }
    ];

    // If dikshit details exist, add them to the array
    if (isDikshit && dikshitDetails && dikshitDetails.length > 0) {
      dikshitDetails.forEach((detail, index) => {
        dataArray.push({
          field: `dikshitDetail_${index + 1}`,
          label: `दीक्षित विवरण ${index + 1}`,
          value: detail,
          displayValue: {
            dikshitName: detail.dikshitName || "N/A",
            worldlyName: detail.worldlyName || "N/A",
            guruName: detail.guruName || "N/A",
            relationship: detail.relationship || "N/A",
            community: detail.community || "N/A"
          }
        });
      });
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const formattedDataArray = formatDataAsArray(values);
      
      // Log the array format for debugging
      console.log("Form data as array:", formattedDataArray);
      
      // Update parent form data with both original format and array format
      const dataToSend = {
        ...values,
        formDataArray: formattedDataArray
      };
      
      updateFormData(dataToSend);
      
      // Show success message
      messageApi.success("फॉर्म सफलतापूर्वक जमा किया गया!");
      
      // Call the handleRegister function provided by the parent
      // Pass the array format to the register function
      handleRegister(formattedDataArray);
    } catch (errorInfo) {
  ...
  const handlePrevStep = () => {
    ...
    // Format as array and include in the data
    const formattedDataArray = formatDataAsArray(currentValues);
    const dataToSend = {
      ...currentValues,
      formDataArray: formattedDataArray
    };
